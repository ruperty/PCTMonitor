...
public class Axis implements Plotable, BaseDependant, Editable {
    ...
    public void plot(AbstractDrawer draw) {
        ...
        if (gridVisible) {
            ...
            for (int i = 0; i < lightLines.length; i++) {
                ...
                // = 1.
                for (int j = base.getAxeScale(index).equalsIgnoreCase(Base.STRINGS) ? 0 : 1; j < lightLines[i].length; j++) {
                    if (draw != null || lightLines[i][j] != null) {
                        try {
            ...
            for (int i = 0; i < lightLabels.length; i++) {
                if (lightLabels[i] == null) {
                    //logger.info("***** lightLabels[i] is null");
                } else {
    ...
    public void setLightLabels() {
        ...
        for (int i = 0; i < lightLabels.length; i++) {
            ...
            lightLabels[i] = new Label(lab, Color.darkGray/*.lightGray*/, labelCoord);
