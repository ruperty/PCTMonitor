 ...
 * @author ReStart
 */
public class XML_060_002_PneuSingleLevelInt {
    ...
    public static void main(String[] args) throws Exception {
                String file = Utils.getFileName("Robot", "PneuControl", "060-002-PneuSingleLevelInt.xml");
        boolean test = true;

        XML_060_002_PneuSingleLevelInt xml = new XML_060_002_PneuSingleLevelInt();
        ...
        globals.put("PneuPositionControl_Gain", "5000");
        globals.put("PneuPositionControl_Slow", "10000000");
        globals.put("PneuPositionControl_Min", "100");
        globals.put("PneuPositionControl_Max", "135");
    ...
    public XML_060_002_PneuSingleLevelInt() {
    ...
    public static Layers.Layer.Controller configureControllerPneuPositionControl() throws Exception {
        ...
        }
        {
            ...
            ControlFunction output = Utils.configureControlFunction("PneuPositionControlOutput", "...", "Integration",
                    new String[][]{
                        {"Gain", Globals.getInstance().get("PneuPositionControl_Gain"), "Double"},
                        {"Min", Globals.getInstance().get("PneuPositionControl_Min"), "Double"},
                        {"Max", Globals.getInstance().get("PneuPositionControl_Max"), "Double"},
                        {"Slow", Globals.getInstance().get("PneuPositionControl_Slow"), "Double"}},
                    null);
            outputFunctions.setOutput(output);
            {
                ...
                List<ControlFunction> transfersList = transfers.getTransfer();
                ControlFunction transfer = Utils.configureControlFunction("VoltageOutput", "", "SerialCSVWrite",
                        new String[][]{{"SensorPort", Globals.getInstance().get("Serial_Port"), ""},
                        {"TimeOut", Globals.getInstance().get("Serial_TimeOut"), "Integer"},
                        {"DataRate", Globals.getInstance().get("Serial_DataRate"), "Integer"}},
                        new String[][]{{"PneuPositionControlOutput"}});
                transfersList.add(transfer);
